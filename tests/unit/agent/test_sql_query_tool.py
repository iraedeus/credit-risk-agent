...
class TestSqlQueryTool:
    def test_sql_query_success(self) -> None:
        """Verify successful SELECT query execution and result table formatting."""
        # 1. Arrange
        mock_conn = MagicMock()
    ...
    def test_sql_query_no_results_or_non_select(self) -> None:
        """Verify handling of successful queries that return no column description (e.g. INSERT)."""
        # 1. Arrange
        mock_conn = MagicMock()
    ...
    def test_sql_query_database_error(self) -> None:
        """Verify that database error messages are returned gracefully without raising unhandled exceptions."""
        # 1. Arrange
        mock_conn = MagicMock()
        mock_conn.__enter__.return_value = mock_conn
        mock_cursor = MagicMock()
    ...
    def test_sql_query_limit(self) -> None:
        """Verify that query results are capped at a maximum of 100 rows."""
        # 1. Arrange
        mock_conn = MagicMock()
        mock_conn.__enter__.return_value = mock_conn
        mock_cursor = MagicMock()
