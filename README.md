...
*   **Инструменты (Tools)**:
    ...
    2.  `run_risk_scoring`: принимает `client_id`, проверяет его наличие в `test_clients.json`, выгружает SQL-запросом историю заемщика, передает тензор в PyTorch-модель, возвращает вероятность дефолта.
