...
from credit_risk_agent.data import preprocess_static
...
class CreditDataset(Dataset[tuple[torch.Tensor, torch.Tensor, torch.Tensor]]):
    ...
    def __init__(self, sequences: np.ndarray, static_features: np.ndarray, labels: np.ndarray) -> None:
        self.sequences = torch.tensor(sequences.copy(), dtype=torch.float32)
        self.static_features = torch.tensor(static_features.copy(), dtype=torch.float32)
